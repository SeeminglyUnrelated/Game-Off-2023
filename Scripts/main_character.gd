...
func _physics_process(delta):
	...
	if Input.is_action_pressed("Down") and $RayDown.is_colliding() and $RayDown.get_collider().is_in_group("OneWayCollisions") and direction == 0:
		position.y += 1
	...
	#if $RayRight.is_colliding() or $RayLeft.is_colliding():
